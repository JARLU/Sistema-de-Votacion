<?php
    $dsn = 'mysql:dbname=sistemavotacion; host=localhost';
    $usuario = 'root';
    $clave = '';   
        try {
  ...
  if ($objeto != NULL) {
    if (is_array($objeto)) {
      ...
      foreach ($objeto as $datos) {
        ...
        $tabla .="</table>";
        $tabla .='<center><a href="Menu.php"><input type="submit" name="menu" value="Menu de Registros" class="google" title="Menu"></a></center>';
    } else {
