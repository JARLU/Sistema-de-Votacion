...
<body onload="javascript:document.forms[0].partidario.disabled=true;" id="candidato">
	...
	<form name="Registro Diputados" id="registro" class="required form-control" action="modificarDiputado.php" method="post" id="registro" class="pager">
		<table>
		<tr>
      <tr>
        <td><input type="hidden" name="id" 
			...
			<td><input type="tex" name="nombres" class="required form-control" style="width:180" title="Ingrese los Nombres" required autocomplete="on" value='<?php print $datos[0][1]?>'></td>
		</tr>
		<tr>
			<td width="120">Apellidos:</td>
			<td><input type="tex" name="apellidos" class="required form-control" style="width:180" title="Ingrese los Apellidos" required autocomplete="on" value='<?php print $datos[0][2]?>'></td>
		</tr>
    <tr>
      <td width="120">N° de DUI:</td>
      <td><input type="text" name="dui" onkeyup="mascara(this,'-',patron3,true)" onkeypress="return justNumbers(event);" maxlength="10" class="required form-control" title="Ingrese el Numero de DUI" style="width:180" required autocomplete="on" placeholder="00000000-0" value='<?php print $datos[0][3]?>'></td>
    </tr>
    <tr>
      ...
      <td><textarea name="direccion" class="required form-control" title="Ingrese su Dirección" style="width:180" required autocomplete="on" value='<?php print $datos[0][4]?>'></textarea></td>
		...
		<tr>
			<td>Partido:</td>
			<td><select style="width:180" class="required form-control" title="Selecionar Partido" name="partido" value='<?php print $datos[0][5]?>'>
		...
		<tr>
			<td>Coalision</td>
			<td><select style="width:180" title="Selecionar Partidos" id="depto" name="coalision" class="required form-control" value='<?php print $datos[0][6]?>'>
				<option value="">Seleccionar...</option>
				<?php
                  $result = $conexion->query("SELECT * FROM inscri_partido");
                  if ($result->num_rows > 0) {
		...
		<tr>
			<td>Departamento:</td>
			<td><select name="departamento" style="width:180" id="depto" class="required form-control" onkeydown="return validarLetras(event)" required="true" title="Selecionar Departamento" value='<?php print $datos[0][7]?>'>
				<option value="">Seleccionar...</option>
				  <?php
